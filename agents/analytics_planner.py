...
from core.schemas import AgentResult, AnalyticsPlannerOutput, BusinessProblemInput
...
def run(business_input: BusinessProblemInput, prior_results: list[AgentResult], config: AppConfig) -> AgentResult:
    ...
    template = load_prompt_template(config.prompts_dir, "analytics_planner")
    prompt = render_prompt(template, business_input, prior_results, AnalyticsPlannerOutput)
    raw_output = call_llm(system_prompt, prompt, config=config)
    parsed = parse_model_output(raw_output, AnalyticsPlannerOutput)
    return AgentResult(
        key="analytics_plan",
