...
from core.schemas import AgentResult, AnalysisPlanOutput, BusinessProblemInput
...
def run(business_input: BusinessProblemInput, prior_results: list[AgentResult], config: AppConfig) -> AgentResult:
    ...
    template = load_prompt_template(config.prompts_dir, "analysis_plan")
    prompt = render_prompt(template, business_input, prior_results, AnalysisPlanOutput)
    raw_output = call_llm(system_prompt, prompt, config=config)
    parsed = parse_model_output(raw_output, AnalysisPlanOutput)
    return AgentResult(
        key="analysis_plan",
        title="Analysis Plan",
