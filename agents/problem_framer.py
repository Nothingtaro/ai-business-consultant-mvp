...
from core.schemas import AgentResult, BusinessProblemInput, ProblemFramingOutput
...
def run(business_input: BusinessProblemInput, prior_results: list[AgentResult], config: AppConfig) -> AgentResult:
    ...
    template = load_prompt_template(config.prompts_dir, "problem_framing")
    prompt = render_prompt(template, business_input, prior_results, ProblemFramingOutput)
    raw_output = call_llm(system_prompt, prompt, config=config)
    parsed = parse_model_output(raw_output, ProblemFramingOutput)
    return AgentResult(
        key="problem_framing",
