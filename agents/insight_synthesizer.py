...
from core.schemas import AgentResult, BusinessProblemInput, InsightSynthesisOutput
...
def run(business_input: BusinessProblemInput, prior_results: list[AgentResult], config: AppConfig) -> AgentResult:
    ...
    template = load_prompt_template(config.prompts_dir, "insight_synthesizer")
    prompt = render_prompt(template, business_input, prior_results, InsightSynthesisOutput)
    raw_output = call_llm(system_prompt, prompt, config=config)
    parsed = parse_model_output(raw_output, InsightSynthesisOutput)
    return AgentResult(
